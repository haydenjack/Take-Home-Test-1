...
def test_sum_current_time_invalid_str():
    "Tests invalid str input raises an ValueError."
...
def test_sum_current_time_list_input():
    "Tests invalid list input raises an ValueError."
    with pytest.raises(TypeError):
        sum_current_time(["1","03","04"])


def test_sum_current_time_empty_input():
    "Tests empty input raises an ValueError."
    with pytest.raises(ValueError):
        sum_current_time("")


def test_sum_current_time_negative_time():
    "Tests invalid time format input raises an ValueError."
    with pytest.raises(ValueError):
        sum_current_time("-1:03:03")
